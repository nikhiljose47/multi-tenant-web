...
@Component({
  ...
  template: `
    <app-site-shell [tenant]="tenant">
      <header
        id="top"
        class="hero space-hero"
        [style.background-image]="'linear-gradient(180deg, rgba(28,25,23,.15), rgba(28,25,23,.65)), url(' + (tenant.coverImage ?? defaultCover) + ')'"
      >
        <div class="hotel-hero-copy">
          <span class="eyebrow">{{ tenant.content.eyebrow ?? 'Spaces worth settling into' }}</span>
...
export class SpaceLayoutComponent {
  ...
  readonly defaultCover = DEFAULT_SPACE_COVER;
}
